...
def attention_sparse(P,Q,Z, activation = None):
    B= Z.shape[0]
    N = Z.shape[1]-1
    d = Z.shape[2]-1
...
class Transformer_F_w_sparse(nn.Module):
    def __init__(self, n_layer, n_head, d, var):
        super(Transformer_F_w_sparse, self).__init__()
    ...
    def forward(self, Z):
        for i in range(self.n_layer):
            ...
            for j in range(self.n_head):
                ...
                residues = residues + attention_sparse(Pij,Qij,Zi)
            Z = Zi + residues
        return Z
...
def attention_double(P,Q,Z, activation = None):
    B= Z.shape[0]
    N = Z.shape[1]-1
    d = Z.shape[2]-1
    A = torch.eye(N+1).double().to(device)
    A[N,N] = 0
    Attn = torch.einsum('BNi, ij, BMj -> BNM', (Z,Q,Z))
    if activation is not None:
        Attn = activation(Attn)
...
class Transformer_F_w_embed(nn.Module):
    def __init__(self, n_layer, n_head, d, var, hidden_dim, device):
        super(Transformer_F_w_embed, self).__init__()
        self.register_parameter('allparam', torch.nn.Parameter(torch.zeros(n_layer, n_head, 2, d+1+hidden_dim, d+1+hidden_dim)))
        with torch.no_grad():
            self.allparam.normal_(0, var)
        for p in self.allparam:
            print(p.shape)
        self.n_layer = n_layer
        self.n_head = n_head
        self.embed = nn.Linear(d+1, hidden_dim)
        self.hidden_dim = hidden_dim
        self.device = device
        
    def forward(self, Z):
        # Z: B, N+1, d+1   embedding: B, N, hidden_dim
        embedding = self.embed(Z[:, :-1, :])
        #print(Z.shape, embedding.shape)
        place_holder = torch.ones(Z.shape[0], 1, self.hidden_dim).to(self.device)
        # place_holder:  B, 1, hidden_dim
        # embedding:   B, N+1, hidden_dim
        embedding = torch.cat((embedding, place_holder), dim=1)
        #print(place_holder.shape, embedding.shape)
        Z = torch.cat((Z, embedding), dim=2)
        #print(Z.shape)
        for i in range(self.n_layer):
            Zi = Z
            residues = 0
...
class Transformer_F_w_embed_mlp(nn.Module):
    def __init__(self, n_layer, n_head, d, var, hidden_dim, device):
        super(Transformer_F_w_embed_mlp, self).__init__()
        self.register_parameter('allparam', torch.nn.Parameter(torch.zeros(n_layer, n_head, 2, d+1+hidden_dim, d+1+hidden_dim)))
        with torch.no_grad():
            self.allparam.normal_(0, var)
        for p in self.allparam:
        ...
        self.n_layer = n_layer
        self.n_head = n_head
        self.embed = nn.Sequential(nn.Linear(d+1, hidden_dim*3), 
                                   nn.ReLU(), 
...
class Transformer_F_w_embed_mlp_double(nn.Module):
    def __init__(self, n_layer, n_head, d, var, hidden_dim, device):
        super(Transformer_F_w_embed_mlp_double, self).__init__()
        self.register_parameter('allparam', torch.nn.Parameter(torch.zeros(n_layer, n_head, 2, d+1+hidden_dim, d+1+hidden_dim)))
        with torch.no_grad():
            self.allparam.normal_(0, var)
        for p in self.allparam:
        ...
        self.embed = nn.Sequential(nn.Linear(d+1, hidden_dim*3), 
                                   nn.ReLU(), 
                                    ...
                                    nn.ReLU(), 
                                    nn.Linear(hidden_dim*3, hidden_dim))   
        print("-------------------->Transformer_F_w_embed_mlp_double", self.embed)
        self.hidden_dim = hidden_dim
        self.device = device
        
    def forward(self, Z, record_attn=False):
        # Z: B, N+1, d+1   embedding: B, N, hidden_dim
        embedding = self.embed(Z[:, :-1, :])
        #print(Z.shape, embedding.shape)
        place_holder = torch.ones(Z.shape[0], 1, self.hidden_dim).to(self.device)
        ...
        for i in range(self.n_layer):
            ...
            for j in range(self.n_head):
                Pij = self.allparam[i,j,0,:,:]
                Qij = self.allparam[i,j,1,:,:]
                attn_output, attn = attention_double(Pij,Qij,Zi)
                residues = residues + attn_output
                if record_attn:
                    attn_dict[f'layer_{i}_head_{j}'] = attn.detach().cpu()
            Z = Zi + residues
        if record_attn:
            return Z, attn_dict
        else:
...
class Transformer_F_w_embed_mlp_v2(nn.Module):
    def __init__(self, n_layer, n_head, d, var, hidden_dim, device):
        super(Transformer_F_w_embed_mlp_v2, self).__init__()
        self.register_parameter('allparam', torch.nn.Parameter(torch.zeros(n_layer, n_head, 2, d+1+hidden_dim, d+1+hidden_dim)))
        with torch.no_grad():
            self.allparam.normal_(0, var)
        for p in self.allparam:
        ...
        self.n_layer = n_layer
        self.n_head = n_head
        self.embed = nn.Sequential(nn.Linear(d, hidden_dim*3), 
                                   nn.ReLU(), 
                                   nn.Linear(hidden_dim*3, hidden_dim*3), 
                                   nn.ReLU(), 
                                   nn.Linear(hidden_dim*3, hidden_dim))   
...
def in_context_solver_loss(model, Z, x, dim, criterion):
    N = Z.shape[1]-1
    output = model(Z)
    mse_loss = criterion(output[:, N, :dim+1], x)
